...
public class SwapScaleLoading_z3 : MonoBehaviour
{
	...
	void Update(){
		...
		float scale = SimpleTween.Linear(currentTime, _minScale, _maxScale, _duration);

		if(_revert){
			_icon2.localScale = new Vector3 (scale, scale, scale);
			_icon1.localScale = new Vector3 (_maxScale - scale, _maxScale - scale, _maxScale - scale);
		}
		else{
